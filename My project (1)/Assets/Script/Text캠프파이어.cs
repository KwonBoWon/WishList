...
public class Text캠프파이어 : MonoBehaviour
{
    ...
    IEnumerator TextPractice()
    {
        ...
        yield return StartCoroutine(NormalChat("곽두팔", "아 이제야 살겠네", false));
        yield return StartCoroutine(NormalChat("곽두팔", "오늘은 너무 힘든 하루였다", false));
        if(data.cross == 2){
            yield return StartCoroutine(NormalChat("곽두팔", "오늘이 지나면 나를 기억해줄 사람이 있을까...", true));
        }
        else{
            yield return StartCoroutine(NormalChat("곽두팔", "이제 버스타고 가자", true));
        }
