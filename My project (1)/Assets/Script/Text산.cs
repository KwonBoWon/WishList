...
public class Text산 : MonoBehaviour
{
    ...
    {   
        ...
        //}
        if(data.tree ==2){//꿀
            change.GetComponent<ChangeScene>().sceneName = "사과나무";
        }
    }
    ...
    IEnumerator TextPractice()
    {
        ...
        yield return StartCoroutine(NormalChat("곽두팔", "여긴 완전 산이네", false));

        yield return StartCoroutine(NormalChat("곽두팔", "날씨도 좀 추워지는거같고", false));

        yield return StartCoroutine(NormalChat("곽두팔", "조금만 더 올라가 보자", true));
    }
