...
public class Text시비 : MonoBehaviour
{
    ...
    {
        yield return StartCoroutine(NormalChat("제갈명식", "와서 부딪혀놓고 뭐라는거에요!", false));
        yield return StartCoroutine(NormalChat("곽두팔", "나 지금 바쁘니까 비키라고", false));
        yield return StartCoroutine(NormalChat("제갈명식", "안전요원을 부를거에요!", false));
        yield return StartCoroutine(NormalChat("곽두팔", "어?.. 그건 안되는데", true));
    }
