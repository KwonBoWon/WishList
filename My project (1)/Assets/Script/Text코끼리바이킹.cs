...
public class Text코끼리바이킹 : MonoBehaviour
{
    ...
    {   
        ...
        //}
        if(data.elephant ==2){//꿀
            change.GetComponent<ChangeScene>().sceneName = "코끼리꿀";
        }
    }
    ...
    {
        yield return StartCoroutine(NormalChat("코끼리", "뿌우?", false));
        yield return StartCoroutine(NormalChat("곽두팔", "왜,, 왜이러지??", false));
        yield return StartCoroutine(NormalChat("코끼리", "뿌우~!!!!", false));
        yield return StartCoroutine(NormalChat("곽두팔", "이 바이킹모자 때문인가?", false));
        yield return StartCoroutine(NormalChat("곽두팔", "그러고보니 이 코끼리 상아가 없어!", false));
        yield return StartCoroutine(NormalChat("코끼리", "뿌우~!!!!", false));
        yield return StartCoroutine(NormalChat("곽두팔", "다.. 다가오지마!!", true));
