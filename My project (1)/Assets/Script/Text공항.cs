...
public class Text공항 : MonoBehaviour
{
    ...
    IEnumerator TextPractice()
    {
        ...
        yield return StartCoroutine(NormalChat("곽두팔", "제일 빨리갈수 있는걸로 하나 주세요", false));
        yield return StartCoroutine(NormalChat("승무원", "지금 당장 출발하는걸로요?", false));
        yield return StartCoroutine(NormalChat("곽두팔", "네 지금 당장 갈 수 있는걸로요", false));
        yield return StartCoroutine(NormalChat("승무원", "5분뒤에 출발하는 비행기로 끊어드릴께요", false));
        yield return StartCoroutine(NormalChat("곽두팔", "네 감사합니다", true));
