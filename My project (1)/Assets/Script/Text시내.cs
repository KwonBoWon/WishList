...
public class Text시내 : MonoBehaviour
{
    ...
    {
        yield return StartCoroutine(NormalChat("곽두팔", "여기가 번화가인가?", false));
        yield return StartCoroutine(NormalChat("곽두팔", "왠 기자가있지? 뉴스촬영중인가?", false));
        yield return StartCoroutine(NormalChat("기자", "현재 도심에 코끼리가 탈출하여 대피령이 내려졌습니다.", true));
